...
def get_file_path(request, filename):
    ...
    filename = "%s%s" % (nowTime, original_filename)   #concatenating the variables in string format and in the filename variable 
...
class Equipment(models.Model): 
    ...
    image = models.ImageField(upload_to='equipment_images', blank=True, validators=[validate_file_size, FileExtensionValidator(['jpg', 'png'])])
    availability_status = models.BooleanField(default=True)
    quantity = models.IntegerField()
    hourly_rate = models.DecimalField(max_digits=10, decimal_places=2)
    user_manual = models.FileField(upload_to='equipment_manuals', blank=True, validators=[validate_file_size, FileExtensionValidator(['pdf', 'docx'])])
